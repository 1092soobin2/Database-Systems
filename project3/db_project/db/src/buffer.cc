...
pagenum_t buffer_alloc_page(table_id_t table_id) {
    ...
    // TODO: 여기서 헤더 페이지 변경되는데 디스크에만 반영되고 버퍼에는 반영이 안 됨.
    //      그럼 반영을 해 주면 되지!!!
    ...
    for (header_bufnum = buffer_info.LRU_head_bufnum; header_bufnum != INVALID_BUFNUM; header_bufnum = buffer_cntl_blocks[header_bufnum].LRU_next_bufnum) 
        if (table_id == buffer_cntl_blocks[header_bufnum].table_id && 0 == buffer_cntl_blocks[header_bufnum].pagenum) {
            ...
            if (verbose) {
                ...
                printf(" (header_page(%lld %lld %lld) sync.ed imm.)", header_page->first_free_pagenum, header_page->number_of_pages, header_page->root_pagenum);
            }
            break;
        }
    ...
    if (verbose) {
        printf("\tpagenum: %lld, bufnum: %d", pagenum, bufnum);
        printf("|");
    }
    return pagenum;
}
...
void buffer_free_page(table_id_t table_id, pagenum_t pagenum) {
    ...
    if (verbose) {
        printf(" |buffer_free_page p: %lld", pagenum);
    }
...
int buffer_request_page(table_id_t table_id, pagenum_t pagenum, header_page_t*& page, int* bufnum) {
    ...
    *bufnum = temp_bufnum;
    ...
    if (verbose) {
        printf(" p: %lld, b: %d", pagenum, *bufnum);
        printf("|");
    }
    return OP_SUCCESS;
}
...
void buffer_release_page(int bufnum, bool is_dirty) {
    ...
    if (buffer_cntl_blocks[bufnum].pagenum == 0 && is_dirty == true) {
        if (verbose) {
            ...
            printf(" (header_page(%lld %lld %lld) flushed imm.)", header_page->first_free_pagenum, header_page->number_of_pages, header_page->root_pagenum);
        }
