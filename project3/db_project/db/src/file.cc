...
int64_t file_open_table_file(const char* pathname) {
    ...
    if (is_new_file != 0) {
        ...
        wrapper_write(fd, 0, header_page);
        ...
        wrapper_write(fd, free_pagenum, free_page);
    }
...
pagenum_t file_alloc_page(table_id_t table_id) {
    ...
    if (verbose) {
        printf(" first_free_pn: %lld, next: %lld", first_free_pagenum, header_page->first_free_pagenum);
    }
    wrapper_write(fd, 0, header_page);

    // Initialize the first free page that will be allocated.
    memset (first_free_page, 0, PAGE_SIZE);
    wrapper_write(fd, first_free_pagenum, first_free_page);
...
void file_free_page(table_id_t table_id, pagenum_t pagenum) {
    ...
    // Write the pages.
    wrapper_write(fd, 0, header_page);
    wrapper_write(fd, pagenum, new_free_page);
