...
pagenum_t find_leaf_pagenum(table_id_t table_id, pagenum_t root_pagenum, page::key_t key) {
    ...
    // Read the root page.
    node_page = (node_page_t*)malloc(PAGE_SIZE);
    if (node_page == NULL) {
        perror("Memory allocation error");
        exit(EXIT_FAILURE);
    }
    file_read_page(table_id, root_pagenum, (page_t*)node_page);
    ...
    while (node_page->header.is_leaf == 0) {
        ...
        // Read the child page.
        file_read_page(table_id, node_pagenum, (page_t*)node_page);
    }
...
int db_find(table_id_t table_id, page::key_t key, char *ret_val, uint16_t *val_size) {
    ...
    // Read the header page of the table.
    ...
    leaf_pagenum = find_leaf_pagenum(table_id, header_page->root_pagenum, key);
    ...
    // Read the leaf page.
    leaf_page = (node_page_t*)malloc(PAGE_SIZE);
    if (leaf_page == NULL) {
        perror("Memory allcation error");
        exit(EXIT_FAILURE);
    }
    ...
    for (slot_index = 0; slot_index < leaf_page->header.number_of_keys; slot_index++) {
        ...
        if (leaf_page->slots[slot_index].key == key) {
            existence_flag = true;
            slot = leaf_page->slots[slot_index];
            memcpy(ret_val, &(leaf_page->values[VALUE_OFFSET(slot.offset)]), slot.size);
            // TODO: is it OKAY? 노노 안 괜찮음... 얘네는 free 되잖아;
...
pagenum_t make_node_page(table_id_t table_id, int is_leaf) {
    ...
    // Read the page.
    new_page = (node_page_t*)malloc(PAGE_SIZE);
    if (new_page == NULL) {
        perror("Memory allcation error");
        exit(EXIT_FAILURE);
    }
    file_read_page(table_id, new_pagenum, (page_t*)new_page);
    ...
    // Write the page.
    file_write_page(table_id, new_pagenum, (page_t*)new_page);
    free(new_page);
...
int insert_into_leaf_page(table_id_t table_id, pagenum_t leaf_pagenum, slot_t slot, char* value) {
    ...
    if (verbose) {
        printf("\t|insert_into_leaf_page (%lld, %d) in %lld\n", slot.key, slot.size, leaf_pagenum);
    ...
    // Write the page.
    file_write_page(table_id, leaf_pagenum, (page_t*)leaf_page);
    free(leaf_page);
...
int insert_into_leaf_page_after_splitting(table_id_t table_id, pagenum_t leaf_pagenum, slot_t slot, char* value) {
    ...
    if (verbose) {
        printf("\t|insert_into_leaf_page_after_splitting (%lld, %d) in %lld\n\t", slot.key, slot.size, leaf_pagenum);
    }
    ...
    // Read the new leaf page.
    new_leaf_page = (node_page_t*)malloc(PAGE_SIZE);
    if (new_leaf_page == NULL) {
        perror("Memory allcation error");
        exit(EXIT_FAILURE);
    }
    file_read_page(table_id, new_leaf_pagenum, (page_t*)new_leaf_page);
    ...
    file_write_page(table_id, leaf_pagenum, (page_t*)leaf_page);
    free(leaf_page);
    file_write_page(table_id, new_leaf_pagenum, (page_t*)new_leaf_page);
    free(new_leaf_page);
...
int insert_into_internal_page(table_id_t table_id, pagenum_t internal_pagenum, page::key_t new_right_key, pagenum_t new_branch_pagenum) {
    ...
    if (verbose) {
        printf(" |insert_into_internal_page (%lld, %lld) in page %lld", new_right_key, new_branch_pagenum, internal_pagenum);
    }
    // Read the internal page.
    internal_page = (node_page_t*)malloc(PAGE_SIZE);
    if (internal_page == NULL) {
        perror("Memory allcation error");
        exit(EXIT_FAILURE);
    }
    file_read_page(table_id, internal_pagenum, (page_t*)internal_page);
    ...
    // Write the page.
    file_write_page(table_id, internal_pagenum, (page_t*)internal_page);
    free(internal_page);
...
int insert_into_internal_page_after_splitting(table_id_t table_id, pagenum_t internal_pagenum, page::key_t new_right_key, pagenum_t new_branch_pagenum) {
    ...
    if (verbose)
        printf(" |insert_into_internal_page_after_splitting (%lld, %lld) in page %lld", new_right_key, new_branch_pagenum, internal_pagenum);
    ...
    // Read the new internal page.
    new_internal_page = (node_page_t*)malloc(PAGE_SIZE);
    if (new_internal_page == NULL) {
        perror("Memory allcation error");
        exit(EXIT_FAILURE);
    }
    file_read_page(table_id, new_internal_pagenum, (page_t*)new_internal_page);
    ...
    // Set the parent pagent number of moved branch pages.
    branch_page = (node_page_t*)malloc(PAGE_SIZE);
    if (branch_page == NULL) {
        perror("Memory allcation error");
        exit(EXIT_FAILURE);
    }
    file_read_page(table_id, new_internal_page->header.branch_first_pagenum, (page_t*)branch_page);
    branch_page->header.parent_pagenum = new_internal_pagenum;
    file_write_page(table_id, new_internal_page->header.branch_first_pagenum, (page_t*)branch_page);
    for (branch_index = 0; branch_index < new_internal_page->header.number_of_keys; branch_index++) {
        ...
        file_read_page(table_id, new_internal_page->branchs[branch_index].pagenum, (page_t*)branch_page);
        branch_page->header.parent_pagenum = new_internal_pagenum;
        file_write_page(table_id, new_internal_page->branchs[branch_index].pagenum, (page_t*)branch_page);
    }
    free(branch_page);
    ...
    file_write_page(table_id, internal_pagenum, (page_t*)internal_page);
    free(internal_page);
    file_write_page(table_id, new_internal_pagenum, (page_t*)new_internal_page);
...
int insert_into_parent_page(table_id_t table_id, pagenum_t branch_pagenum, page::key_t new_right_key, pagenum_t new_branch_pagenum) {
    ...
    if (verbose)
        printf(" |insert_into_parent_page->");
    ...
    file_read_page(table_id, branch_pagenum, (page_t*)branch_page);
    ...
    parent_page = (node_page_t*)malloc(PAGE_SIZE);
    if (parent_page == NULL) {
        perror("Memory allcation error");
        exit(EXIT_FAILURE);
    }
    file_read_page(table_id, parent_pagenum, (page_t*)parent_page);
...
int insert_into_new_root_page(table_id_t table_id, pagenum_t branch_pagenum, page::key_t new_right_key, pagenum_t new_branch_pagenum) {
    ...
    if (verbose) 
        printf(" |insert_into_new_root_page");

    // Make a new root page which is a internal page.
    root_pagenum = make_node_page(table_id, 0);
    // Read the root page.
    root_page = (node_page_t*)malloc(PAGE_SIZE);
    if (root_page == NULL) {
        perror("Memory allcation error");
        exit(EXIT_FAILURE);
    }
    file_read_page(table_id, root_pagenum, (page_t*)root_page);
    ...
    // Write the root page.
    file_write_page(table_id, root_pagenum, (page_t*)root_page);
    free(root_page);
    ...
    file_write_page(table_id, branch_pagenum, (page_t*)branch_page);
    file_read_page(table_id, new_branch_pagenum, (page_t*)branch_page);
    branch_page->header.parent_pagenum = root_pagenum;
    file_write_page(table_id, new_branch_pagenum, (page_t*)branch_page);
    free(branch_page);
...
int db_insert(int64_t table_id, int64_t key, char* value, uint16_t val_size) {
    ...
    if (verbose) {
        printf("(num_keys:%d)", leaf_page->header.number_of_keys);
    }
    // Find the key in the leaf page.
    for (slot_index = 0; slot_index < leaf_page->header.number_of_keys; slot_index++)
    ...
    if (verbose) {
        printf("-> %lld, %d\n", leaf_page->header.amount_of_free_space, split_flag);
    }

    // Deallocate the leaf page.
    free(leaf_page);


    // II. leaf has room for insertion.
    if (split_flag == false)
        return insert_into_leaf_page(table_id, leaf_pagenum, slot, value);
    // III. leaf does not have room for insertion.
    else   
...
int remove_entry_from_internal_page(table_id_t table_id, pagenum_t internal_pagenum, page::key_t key) {
    ...
    // Read the internal page.
    wrapper_file_read_page(table_id, internal_pagenum, internal_page);
    ...
    // Write the internal page.
    wrapper_file_write_page(table_id, internal_pagenum, internal_page);
...
int adjust_root_page(table_id_t table_id, pagenum_t root_pagenum) {
    ...
    // Read the root page.
    wrapper_file_read_page(table_id, root_pagenum, root_page);
    ...
    wrapper_file_read_page(table_id, 0, header_page);
    ...
    if (root_page->header.is_leaf == 0) {
        ...
        wrapper_file_write_page(table_id, 0, header_page);

        // Modify the metadata in the new root_page.
        wrapper_file_read_page(table_id, new_root_pagenum, new_root_page);
        new_root_page->header.parent_pagenum = 0;
        // Write the new root page.
        wrapper_file_write_page(table_id, new_root_pagenum, new_root_page);
...
int coalesce_leaf_pages(table_id_t table_id, pagenum_t leaf_pagenum, pagenum_t neighbor_pagenum, page::key_t key_between_two) {
    ...
    if (verbose) {
        printf(" |coalesce_leaf_pages(nei -key- leaf) %lld -%lld- %lld \n", neighbor_pagenum, key_between_two, leaf_pagenum);
        printf("\tbefore");
    ...
    // Read the pages.
    wrapper_file_read_page(table_id, leaf_pagenum, leaf_page);
    wrapper_file_read_page(table_id, neighbor_pagenum, neighbor_page);
    ...
    if (leaf_page->slots[0].key < neighbor_page->slots[0].key) {
        tmp_pagenum = leaf_pagenum;
        leaf_pagenum = neighbor_pagenum;
        neighbor_pagenum = tmp_pagenum;
        // Read the pages.
        file_read_page(table_id, leaf_pagenum, (page_t*)leaf_page);
        file_read_page(table_id, neighbor_pagenum, (page_t*)neighbor_page);
    }
    ...
    // Write the neighbor page.
    file_write_page(table_id, neighbor_pagenum, (page_t*)neighbor_page);
    free(neighbor_page);
    ...
    // Free the leaf page.
    free(leaf_page);
    file_free_page(table_id, leaf_pagenum);
...
int coalesce_internal_pages(table_id_t table_id, pagenum_t internal_pagenum, pagenum_t neighbor_pagenum, page::key_t key_between_two) {
    ...
    if (verbose) {
        printf(" |coalesce_internal_pages (nei -key- intern) %lld -%lld- %lld\n", neighbor_pagenum, key_between_two, internal_pagenum);
        printf("\tbefore");
        print_page(table_id, neighbor_pagenum);
        print_page(table_id, internal_pagenum);
    }
    ...
    wrapper_file_read_page(table_id, neighbor_page->branchs[appending_index].pagenum, branch_page);
    branch_page->header.parent_pagenum = neighbor_pagenum;
    wrapper_file_write_page(table_id, neighbor_page->branchs[appending_index].pagenum, branch_page);
    // Modify the metadata.
    neighbor_page->header.number_of_keys++;
    for (neighbor_index = appending_index + 1, internal_index = 0; internal_index < internal_page->header.number_of_keys; neighbor_index++, internal_index++) {
        ...
        // Update parent_pagenum of moved branchs. (Read, modify, write)
        wrapper_file_read_page(table_id, neighbor_page->branchs[neighbor_index].pagenum, branch_page);
        branch_page->header.parent_pagenum = neighbor_pagenum;
        wrapper_file_write_page(table_id, neighbor_page->branchs[neighbor_index].pagenum, branch_page);
    ...
    // Write the neighbor page.
    wrapper_file_write_page(table_id, neighbor_pagenum, neighbor_page);

    // Get the parent page number.
    parent_pagenum = internal_page->header.parent_pagenum;

    // Free the internal page.
    free(internal_page);
    file_free_page(table_id, internal_pagenum);
...
int redistribute_leaf_pages(table_id_t table_id, pagenum_t leaf_pagenum, pagenum_t neighbor_pagenum, int branch_index_between_two) {
    ...
    if (verbose) {
        printf(" |redistribute_leaf_pages(nei -idx- leaf) %lld -%d- %lld \n", neighbor_pagenum, branch_index_between_two, leaf_pagenum);
        printf("\tbefore redist");
        print_page(table_id, neighbor_pagenum);
        print_page(table_id, leaf_pagenum);
    }

    // Read the leaf page and neighbor page.
    leaf_page = (node_page_t*)malloc(PAGE_SIZE);
    if (leaf_page == NULL) {
        perror("Memory allcation error");
        exit(EXIT_FAILURE);
    ...
    file_read_page(table_id, leaf_pagenum, (page_t*)leaf_page);
    neighbor_page = (node_page_t*)malloc(PAGE_SIZE);
    ...
    if (neighbor_page->slots[0].key < leaf_page->slots[0].key) {
        ...
        while (leaf_page->header.amount_of_free_space >= THRESHOLD) {
            ...
            // Read the new leaf page and neighbor page.
            file_read_page(table_id, leaf_pagenum, (page_t*)leaf_page);
            file_read_page(table_id, neighbor_pagenum, (page_t*)neighbor_page); 
        }
        ...
        file_write_page(table_id, parent_pagenum, (page_t*)parent_page);
        free(parent_page);
    }
...
int redistribute_internal_pages(table_id_t table_id, pagenum_t internal_pagenum, pagenum_t neighbor_pagenum, int branch_index_between_two) {
    ...
    if (verbose) {
        printf(" |redistribute_internal_pages(nei -idx- leaf) %lld -%d- %lld \n", neighbor_pagenum, branch_index_between_two, internal_pagenum);
        printf("\tbefore redist");
        print_page(table_id, neighbor_pagenum);
        print_page(table_id, internal_pagenum);
    }

    // Read the internal page and neighbor page.
    wrapper_file_read_page(table_id, internal_pagenum, internal_page);
    wrapper_file_read_page(table_id, neighbor_pagenum, neighbor_page); 

    // Read the parent page.
    parent_pagenum = internal_page->header.parent_pagenum;
    wrapper_file_read_page(table_id, parent_pagenum, parent_page);
    ...
    if (neighbor_page->branchs[0].key < internal_page->branchs[0].key) {
        ...
        wrapper_file_read_page(table_id, internal_page->header.branch_first_pagenum, branch_page);
        branch_page->header.parent_pagenum = internal_pagenum;
        wrapper_file_write_page(table_id, internal_page->header.branch_first_pagenum, branch_page);
    ...
    // II. internal -branch- neighbor
        ...
        // Update parent_pagenum of moved pagenum. (Read, modify, write)
        wrapper_file_read_page(table_id, internal_page->branchs[internal_page->header.number_of_keys].pagenum, branch_page);
        branch_page->header.parent_pagenum = internal_pagenum;
        wrapper_file_write_page(table_id, internal_page->branchs[internal_page->header.number_of_keys].pagenum, branch_page);
    ...
    // Write the pages.
    wrapper_file_write_page(table_id, parent_pagenum, parent_page);
    wrapper_file_write_page(table_id, internal_pagenum, internal_page);
    wrapper_file_write_page(table_id, neighbor_pagenum, neighbor_page);
...
int delete_entry_in_page(table_id_t table_id, pagenum_t pagenum, int is_leaf, page::key_t key) {
    ...
    if (verbose) {
        printf(" |delete_entry_in_page %ld in %lld ", key, pagenum);
    }
    ...
    // II. the page is not the root page. (merge | redistribution)
    // Read the page.
    wrapper_file_read_page(table_id, pagenum ,page);
    // II-1. The page is a leaf page.
    if (is_leaf == 1) {
        // i. free_space < threshold
        ...
        // ii. free_space >= threshold, structure modification needed.
            ...
            // Deallocate the parent page.
            free(parent_page);

            // Read the neighbor page.
            wrapper_file_read_page(table_id, neighbor_pagenum, neighbor_page);
...
int db_delete(int64_t table_id, int64_t key) {
    ...
    if (verbose) {
        printf("before deletion, delete_flag: %d", delete_flag);
        print_page(table_id, leaf_pagenum);
...
int init_db(void) {
    file_init();
    // TODO: should be changed to false
    verbose = true;
    return OP_SUCCESS;
}
...
inline void wrapper_file_read_page(table_id_t table_id, pagenum_t pagenum, header_page_t*& dst) {
    dst = (header_page_t*)malloc(PAGE_SIZE);
    if (dst == NULL) {
        perror("Memory allcation error");
        exit(EXIT_FAILURE);
    }
    file_read_page(table_id, pagenum, (page_t*)dst);
}
inline void wrapper_file_read_page(table_id_t table_id, pagenum_t pagenum, node_page_t*& dst) {
    // TODO: malloc failure 처리할 떄 그냥 exit 해버려도 되는 건가..?
    dst = (node_page_t*)malloc(PAGE_SIZE);
    if (dst == NULL) {
        perror("Memory allcation error");
        exit(EXIT_FAILURE);
    }
...
inline void wrapper_file_write_page(table_id_t table_id, pagenum_t pagenum, header_page_t*& src) {
    file_write_page(table_id, pagenum, (page_t*)src);
    free(src);
}
...
void print_page(table_id_t table_id, pagenum_t pagenum) {
    ...
    printf("\n[page %lld]-------num_keys: %d----------parent_pn: %lld------------------\n", pagenum, page->header.number_of_keys, page->header.parent_pagenum);
