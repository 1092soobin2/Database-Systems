...
lock_t* lock_acquire(int64_t table_id, pagenum_t page_id, int64_t key, int trx_id, int lock_mode) {
    ...
    if (lock_convert_implicit_to_explicit(table_id, page_id, key) == -1)
        return NULL;
...
int trx_abort(int trx_id) {
    ...
    // Undo.
    while (trx_table[trx_id].log_head != NULL) {
        undo_log = trx_table[trx_id].log_head;

        buffer_request_page(undo_log->table_id, undo_log->page_id, leaf_page, &leaf_bufnum);
        leaf_page->slots[undo_log->slot_id].size = undo_log->old_val_size;
        memcpy(&leaf_page->values[VALUE_OFFSET(leaf_page->slots[undo_log->slot_id].offset)], undo_log->old_value, undo_log->old_val_size);
        buffer_release_page(leaf_bufnum, true);

        // Remove the log from the log list.
        trx_table[trx_id].log_head = undo_log->log_next;
        free(undo_log);
    }
    while (trx_table[trx_id].lock_list_head != NULL) {
        lock_obj = trx_table[trx_id].lock_list_head;
...
int trx_append_log(log_t* log, int trx_id) {
    pthread_mutex_lock(&trx_table_latch);
    if (trx_table[trx_id].log_head == NULL) {
        trx_table[trx_id].log_head = log;
        trx_table[trx_id].log_tail = log;
    } else {
        trx_table[trx_id].log_tail->log_next= log;
        trx_table[trx_id].log_tail = log;
    }
    pthread_mutex_unlock(&trx_table_latch);
}
