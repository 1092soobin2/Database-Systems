...
int db_update(int64_t table_id, int64_t key, char* values, uint16_t new_val_size, uint16_t* old_val_size, int trx_id) {
    ...
    for (slot_index = 0; slot_index < leaf_page->header.number_of_keys; slot_index++) {
        if (key == leaf_page->slots[slot_index].key) {
            ...
            log->old_val_size = leaf_page->slots[slot_index].size;
            memcpy(log->old_value, &(leaf_page->values[VALUE_OFFSET(leaf_page->slots[slot_index].offset)]), log->old_val_size);
            log->new_val_size = new_val_size;
            memcpy(log->new_value, values, log->new_val_size);
            log->table_id = table_id;
            log->page_id = leaf_pagenum;
            log->slot_id = slot_index;
            log->log_next = NULL;
            // Push the log into the log list.
            trx_append_log(log, trx_id);
